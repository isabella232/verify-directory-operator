...
This script is designed to handle the management of the Verify Directory 
operator docker build image and containers.  The usage option can be used to 
provide details on the usage of this script.
"""
...
image_name = "verify-directory-operator-build"
...
This script can be used to manage a docker Verify Directory operator build 
...
3. You need a copy of the verify-directory-operator repository that you are 
   building.  
...
'create': Used to create a new container for a specific workspace.  When the 
          ...
          container will stop once you exit from the shell.  For example:
            'operator_build.py create --container build.operator \
                --source /Users/dev/github/verify-directory-operator'
...
def create_arg_parser():
    ...
    parser = argparse.ArgumentParser(description="This script is designed "
        "to help manage the verify directory operator Docker build container.  "
        "At a high level you should construct the build image in your "
    ...
    cmd = subparsers.add_parser("build", help="This command will " 
        "construct the verify directory operator build image.")
    ...
    cmd.add_argument("-s", "--source", help="The name of the directory "
        "which contains the verify-directory-operator source code.  Default: "
        "current path",
        required=False)

    cmd.add_argument("-c", "--container", help="The name of the container "
        "which will be created. Default: directory.operator.build",
        required=False, default="directory.operator.build")
    ...
    cmd.add_argument("-c", "--container", help="The name of the container used "
        "for this build. Default: directory.operator.build",
        required=False, default="directory.operator.build")
